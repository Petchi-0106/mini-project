...
const RegisterPage = ({ role, formData, setFormData, setView, handleRegister, error }) => (
    ...
    <div className="w-full max-w-md animate-scale-in">
      ...
      <div className="text-center mb-8">
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-4 ${role === 'faculty' ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' : 'bg-violet-50 text-violet-600 border border-violet-100'}`}>
          <Sparkles size={12} /> {role === 'faculty' ? 'Faculty Registration' : 'Student Registration'}
        </div>
        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Create Account</h1>
        <p className="text-slate-400 font-medium mt-2">Join the EventHub ecosystem today</p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl shadow-slate-200 border border-slate-100 p-8">
        ...
        <form className="space-y-5" onSubmit={handleRegister}>
          <div>
            <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.18em] mb-2">Full Name</label>
            <input
              type="text"
              placeholder={role === 'faculty' ? 'Dr. John Smith' : 'Jane Doe'}
              className="input-field w-full p-4 rounded-2xl border border-slate-200 font-medium text-slate-800 placeholder-slate-300"
              onChange={e => setFormData({ ...formData, [role === 'faculty' ? 'facultyName' : 'studentName']: e.target.value })}
              required
            />
          </div>
          ...
          <div>
            <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.18em] mb-2">Email Address</label>
            <input
              type="email"
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 text-center">
          ...
          <button onClick={() => setView('login')} className="text-sm font-black text-indigo-600 hover:text-indigo-800 transition-colors underline underline-offset-2">
            Sign In
          </button>
        </div>
      </div>
